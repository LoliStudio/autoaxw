  ...
  -
    ...
    ControllerConfig:
      ...
      CertConfig:
        CertMode: ${ssl_mode}
        CertDomain: ${domain}
        CertFile: /root/.acme.sh/${rdomain}_ecc/fullchain.cer
        KeyFile: /root/.acme.sh/${rdomain}_ecc/${rdomain}.key
EOF
}
  ...
  cat > /etc/au/au.json << EOF
{
  ...
  "proxy": {
    ...
    "cert": {
      "cert_path": "/root/.acme.sh/${rdomain}_ecc/fullchain.cer",
      "key_path": "/root/.acme.sh/${rdomain}_ecc/${rdomain}.key"
    },
    "speed_limit_level": [0]
  },
  "sync": {
...
server {
  ...
  ssl_certificate /root/.acme.sh/${rdomain}_ecc/fullchain.cer;
  ssl_certificate_key /root/.acme.sh/${rdomain}_ecc/${rdomain}.key;
...
————————————————
  ${green}5.${plain} 安装 SS
————————————————
  ${green}6.${plain} 安装 Warp（${red}仅适用于 Air-Universe${plain}）
————————————————
  ${green}7.${plain} 卸载
————————————————
  ${green}8.${plain} 升级维护脚本
  ${green}9.${plain} 查看后端版本
  "
